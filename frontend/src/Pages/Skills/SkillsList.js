...
function SkillsList(props) {
    ...
    const getSkills = async () => {
        const skils = await axios.get("http://localhost:5000/skills");
            ...
            <table className="table table-striped table-dark">
                ...
                {skills.map((s) => (
                    <tr key={s._id}>
                        ...
                        <td>{s.description}</td>
                        <td>
                            <img src={`http://localhost:5000/uploads/${s.image}`} width={50} alt=""/>
                        </td>
                        <td>
                            <button className="btn btn-danger" onClick={() => handleDelete(s._id)}>delete</button>
                            <Link className="btn btn-secondary" to={`/skill/edit/${s._id}`}>edit</Link>
                        </td>
                    </tr>
                ))}
                </tbody>
